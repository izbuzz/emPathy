// api request response stored here to not use up api limits
...
// we spoof instead of fetch due to api limits
// fetch(url, options)
...
// show first three jobs
json["hits"].slice(0, 3).forEach(job => createListing(job));

showMoreButton.addEventListener("click", (event) => {
  event.preventDefault();
  // no more jobs
  if (amount >= json["hits"].length) {
    showMoreButton.style.display = "none";
    showMoreButton.style.visibility = "hidden";
    return;
  }
  // get the next three jobs
  json["hits"].slice(amount, amount + 3).forEach(job => createListing(job));
  amount += 3;
});

function createListing(job) {
  // element with a box layout
  ...
  const indeedLink = job["link"];
  // sanitize the link for consumption
  const jobLink = indeedLink.slice(5, indeedLink.indexOf("?")); 
  link.href = "https://www.indeed.com/viewjob?jk=" + jobLink + "&";
